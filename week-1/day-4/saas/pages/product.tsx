...
function ConsultationForm() {
    ...
    async function handleSubmit(e: FormEvent) {
        ...
        await fetchEventSource('/api', {
            ...
            onmessage(ev) {
                if (ev.data === "[DONE]") {
                    // Akış bittiğinde yapılacak işlemler (opsiyonel)
                    return;
                }

                try {
                    // Backend'den gelen "\\n" metinlerini gerçek alt satır karakterine (\n) çeviriyoruz
                    const formattedData = ev.data.replace(/\\n/g, '\n');

                    // Eğer çift tırnak sorunu yaşanırsa (bazı durumlarda tırnak içinde gelir)
                    // const cleanData = formattedData.replace(/^"|"$/g, ''); 
