...
async def run_charter_agent(
    job_id: str, portfolio_data: Dict[str, Any], db=None
) -> Dict[str, Any]:
    ...
    with trace("Charter Agent"):
        agent = Agent(
            name="Chart Maker", instructions=CHARTER_INSTRUCTIONS, model=model
        )
        ...
        logger.info(
            f"Charter: Agent completed, output length: {len(output) if output else 0}"
        )
        ...
        if output:
            ...
            if start_idx >= 0 and end_idx > start_idx:
                ...
                logger.info(
                    f"Charter: Extracted JSON substring, length: {len(json_str)}"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Charter: Successfully parsed JSON, found {len(charts)} charts"
                    )
                    ...
                    if charts:
                        ...
                        logger.info(
                            f"Charter: Created charts_data with keys: {list(charts_data.keys())}"
                        )
                        ...
                        if db and charts_data:
                            try:
                                ...
                                logger.info(
                                    f"Charter: Database update returned: {success}"
                                )
        ...
        return {
            ...
            "message": f"Generated {len(charts_data) if charts_data else 0} charts"
            if charts_saved
...
def lambda_handler(event, context):
    ...
    with observe():
        try:
            logger.info(
                f"Charter Lambda invoked with event keys: {list(event.keys()) if isinstance(event, dict) else 'not a dict'}"
            )
            ...
            if not portfolio_data:
                ...
                try:
                    ...
                    if job:
                        ...
                        logger.info(
                            f"Charter: Loaded {len(portfolio_data['accounts'])} accounts with positions"
                        )
                    else:
