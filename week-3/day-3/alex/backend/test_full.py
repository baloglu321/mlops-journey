    ...
    if not accounts:
        ...
        for pos in positions:
            position_data = PositionCreate(
                account_id=account_id, symbol=pos["symbol"], quantity=pos["quantity"]
            )
            db.positions.create(position_data.model_dump())
        print(f"  ✓ Created {len(positions)} positions")
    else:
        print(
            f"  ✓ Test account exists with {len(db.positions.find_by_account(accounts[0]['id']))} positions"
        )
            ...
            if job.get("charts_payload"):
                ...
                for chart_key, chart_data in charts.items():
                    if isinstance(chart_data, dict):
                        ...
                        print(
                            f"   - {chart_key}: {title} ({chart_type}, {data_points} data points)"
                        )
            else:
                print("\n❌ No charts found")
            ...
            if job.get("retirement_payload"):
                ...
                if isinstance(retirement, dict):
                    ...
                    if "projected_balance" in retirement:
                        print(
                            f"   - Projected Balance: ${retirement['projected_balance']:,.0f}"
                        )
                    if "analysis" in retirement:
                        print(
                            f"   - Analysis Length: {len(retirement['analysis'])} characters"
                        )
            else:
