        ...
        try:
            ...
            if response["DBClusters"]:
                ...
                if not cluster.get("HttpEndpointEnabled", False):
                    ...
                    print(
                        "💡 Run: aws rds modify-db-cluster --db-cluster-identifier alex-aurora-cluster --enable-http-endpoint --apply-immediately"
                    )
                    return None, None
            else:
...
def test_data_api(cluster_arn, secret_arn, region):
    ...
    try:
        ...
        if error_code == "BadRequestException":
            ...
            print(
                f"   ⚠️  Database 'alex' might not exist or credentials are incorrect"
            )
    ...
    try:
        ...
        tables = [
            record[0].get("stringValue") for record in response.get("records", [])
        ]
