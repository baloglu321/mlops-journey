    ...
    def get_portfolio_value(self, account_id: str) -> Dict:
        ...
        if result:
            return {
                ...
                "total_shares": float(result.get("total_shares", 0))
...
class Jobs(BaseModel):
    ...
    def create_job(
        self, clerk_user_id: str, job_type: str, request_payload: Dict = None
    ) -> str:
        """Create a new job"""
        data = {
            "clerk_user_id": clerk_user_id,
    ...
    def find_by_user(
        self, clerk_user_id: str, status: str = None, limit: int = 20
    ) -> List[Dict]:
