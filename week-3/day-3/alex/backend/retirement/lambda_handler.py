...
@retry(
    ...
    before_sleep=lambda retry_state: logger.info(
        f"Retirement: Temporary error, retrying in {retry_state.next_action.sleep} seconds..."
    ),
)
async def run_retirement_agent(
    job_id: str, portfolio_data: Dict[str, Any]
) -> Dict[str, Any]:
...
def lambda_handler(event, context):
    ...
    with observe() as observability:
        try:
            logger.info(
                f"Retirement Lambda invoked with event: {json.dumps(event)[:500]}"
            )
            ...
            if not portfolio_data:
                ...
                try:
                    ...
                    if job:
                        ...
                        logger.info(
                            f"Retirement: Loaded {len(portfolio_data['accounts'])} accounts with positions"
                        )
                    else:
